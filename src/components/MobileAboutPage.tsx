// MobileAboutPage.tsx — About me page, Mobile layout (< 1024px)
// Figma: node-id 817-1446
...
const CARDS = [
  {
    num:   '01',
    title: 'Designer',
    tags:  ['Design Thinking', 'Creative Process', 'Collaboration', 'Exploration'],
    gallery: [
      { src: img01Designer1, aspect: '3894/2774' },
      { src: img01Designer2, aspect: '151/201'   },
  ...
  },
  {
    ...
    gallery: [
      { src: img02Explorer1, aspect: '151/201' },
      { src: img02Explorer2, aspect: '151/201' },
      { src: img02Explorer3, aspect: '151/201' },
      { src: img02Explorer4, aspect: '151/201' },
    ],
    galleryGap: 8,
    galleryLayout: 'grid-2x2' as const,
  },
  {
    num:   '03',
    title: 'Curious Human',
    ...
    gallery: [
      { src: img03Curious1, aspect: '310/211' },
      { src: img03Curious2, aspect: '151/201' },
      { src: img03Curious3, aspect: '151/201' },
    ],
    galleryGap: 8,
    galleryLayout: 'default' as const,
  },
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        onClick={() => navigate('/')}
        className="group fixed top-[16px] right-[24px] w-[48px] h-[48px] bg-[#1e1e1e] flex items-center justify-center z-50 border-0 outline-none cursor-pointer shrink-0"
        aria-label="Close"
      >
      ...
      {/* Page content: px-24px pt-80px pb-40px */}
      <div className="w-full px-[24px] pt-[80px] pb-[40px]">
        <div className="w-full flex flex-col gap-[40px] items-start">

          {/* ── Top section ── */}
          <div className="flex flex-col gap-[32px] items-start w-full relative">

            <AboutDeco />

            {/* Profile photo — 120×137px */}
            <div className="w-[120px] h-[137px] relative shrink-0 overflow-hidden">
              <img
                src={imgProfile}
                alt="May Im"
            ...
            {/* Headline — 20px/28px Medium */}
            <div className="w-full">
              <div
                className="text-[#1e1e1e] text-[20px] leading-[0] not-italic w-full"
                style={{ fontWeight: 500 }}
              >
                <p className="leading-[28px] mb-0">I'm May,</p>
                <p className="leading-[28px]">a product designer who listens first and turns what I learn into thoughtful digital experiences.</p>
              </div>
            </div>

            {/* Body text — 14px/21px */}
            <div
              className="text-[#1e1e1e] text-[14px] leading-[21px] not-italic w-full"
          ...
          <div className="flex flex-col gap-[16px] items-start w-full">
            {CARDS.map((card) => (
              <div
                key={card.num}
                className="bg-white flex flex-col gap-[16px] items-start p-[16px] w-full"
              >
                {/* Card header */}
                <div className="flex gap-[16px] items-center w-full">
                  <div className="bg-[#fbf27e] w-[28px] h-[28px] shrink-0 flex items-center justify-center">
                    <p
                      className="text-[#1e1e1e] text-[15px] leading-[22px] not-italic text-center whitespace-nowrap m-0"
                      style={{ fontWeight: 500 }}
                    >
                      {card.num}
                    </p>
                  ...
                  <p
                    className="text-[#1e1e1e] text-[16px] leading-[24px] not-italic flex-1 min-w-0 m-0"
                    style={{ fontWeight: 500 }}
                  >
                    {card.title}
                  </p>
                ...
                <div className="flex gap-[6px] items-center flex-wrap">
                  {card.tags.map((tag) => (
                    <div
                      key={tag}
                      className="bg-[#f7f4f0] px-[10px] py-[4px] shrink-0 flex items-center justify-center"
                    >
                      <p
                        className="text-[#1e1e1e] text-[13px] leading-[19px] not-italic whitespace-nowrap m-0"
                        style={{ fontWeight: 400 }}
                      >
                        {tag}
                      </p>
                ...
                {/* Gallery — grid-2x2: 4장 균등 50%, default: 1st 100% + 나머지 50% */}
                <div
                  className="flex flex-wrap items-start w-full"
                  style={{ gap: `${card.galleryGap}px` }}
                >
                  {card.gallery.map((photo, i) => (
                    <div
                      ...
                      style={{
                        width: card.galleryLayout === 'grid-2x2'
                          ? `calc(50% - ${card.galleryGap / 2}px)`
                          : (i === 0 ? '100%' : `calc(50% - ${card.galleryGap / 2}px)`),
